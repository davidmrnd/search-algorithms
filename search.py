...
def graph_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            print(f"Nodos visitados = {visitado}")
...
def branch_and_bound_graph_search(problem):
    """Searches the shallowest nodes first, ensuring the shortest path is found in an unweighted search tree."""
    return graph_search(problem, BranchAndBoundQueue()) 


def informed_branch_and_bound_graph_search(problem):
    """Searches nodes based on the sum of the path cost and heuristic estimate to the goal, guiding the search towards the most promising paths."""
    return graph_search(problem, InformedBranchAndBoundQueue(problem)) 
